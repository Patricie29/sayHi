...
export async function POST(req: Request) {
    ...
    try {
        ...
        await database.srem(`user:${session.user.id}:incoming_friend_requests`, idToAdd)
        await database.srem(`user:${idToAdd}:incoming_friend_requests`, session.user.id)
